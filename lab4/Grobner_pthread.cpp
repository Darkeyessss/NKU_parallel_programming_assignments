...
void *EliminateUsingNeonThread(void *arg)
{
    ThreadData *data = (ThreadData *)arg;
    unsigned int **R = data->R;
    unsigned int **E = data->E;
    int *firstElement = data->firstElement;
...
void EliminateUsingNeon(unsigned int **R, unsigned int **E, int *firstElement, int numRows)
{
    ...
    int rowsPerThread = numRows / NUM_THREADS;

    for (int i = 0; i < NUM_THREADS; i++)
    {
        threadData[i].R = R;
        threadData[i].E = E;
        threadData[i].firstElement = firstElement;
        threadData[i].startRow = i * rowsPerThread;
        threadData[i].endRow = (i == NUM_THREADS - 1) ? numRows : (i + 1) * rowsPerThread;
        pthread_create(&threads[i], NULL, EliminateUsingNeonThread, (void *)&threadData[i]);
    }
